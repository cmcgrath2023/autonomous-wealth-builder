...
export function ModuleCard({ module }: ModuleCardProps) {
  ...
  return (
    <Link href={`/${module.id}`}>
      <Card className="bg-white/5 border border-white/5 hover:border-white/10 transition-colors cursor-pointer">
        <CardHeader className="flex justify-between items-start pb-0 px-4 pt-4">
          <div>
          ...
          <Chip size="sm" variant="flat" color={statusColor}>{module.status}</Chip>
        </CardHeader>
        <CardBody className="px-4 pb-4 pt-3">
          <div className="flex justify-between items-end">
            <div>
              <div className="text-xs text-white/40">Allocation</div>
              <div className="text-lg font-semibold">{formatCurrency(module.allocation)}</div>
